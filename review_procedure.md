...
**QC confirmation (quick path)**
- Run main.py → pipeline writes QC CSV + readable TXT with timestamp in output/<stage>/.
- Review both files; if satisfactory, rerun and approve the QC prompt (or export CONFIRM_QC_SAMPLE=1) to continue.
- If not, rerun main.py to issue a fresh QC sample; each rerun gets a new timestamp.
