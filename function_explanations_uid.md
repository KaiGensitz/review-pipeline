...
- Human readable hint: current user-configured AI-transparency evidence hints include usage, deployment, workflow, development/training, input-feature, decision-logic, rationale, and cited-source referral terms so the generic selector can find transparent AI-system descriptions without embedding review terms in `pipeline/`.
