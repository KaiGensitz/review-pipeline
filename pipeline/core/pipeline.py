...
from pipeline.core.extraction_io import (
    PerPaperFileIndex,
    SupplementalCitedEvidenceLoader,
...
from pipeline.selection.chunking import ChunkBuilder, chunk_fulltext_sentences, chunk_paper_sentences
...
SECTION_PRIORITY = retrieval_terms("section_priority")
...
METHOD_EVIDENCE_PATTERN = retrieval_pattern("method_evidence_terms")
MONITORING_OR_PROTOCOL_PATTERN = retrieval_prefix_pattern("monitoring_seed_roots")
SUBSTANTIVE_MAIN_TEXT_PATTERN = retrieval_pattern("main_text_evidence_terms")
@dataclass
class PaperRecord:
    paper_id: str
    title: str
...
class FullTextRatioCheckCache:
    ...
    def from_artifact_payload(cls, payload: dict[str, Any] | None) -> "FullTextRatioCheckCache | None":
        ...
        raw = payload.get("full_text_ratio_check") if isinstance(payload, dict) else None
        if not isinstance(raw, dict):
            return None
        try:
            values = {
                "pdf_size": int(raw["pdf_size"]),
                "pdf_mtime_ns": int(raw["pdf_mtime_ns"]),
                "normalized_text_sha256": str(raw["normalized_text_sha256"]).strip(),
                "normalized_len": int(raw["normalized_len"]),
                "direct_len": int(raw["direct_len"]),
                "ratio": float(raw["ratio"]),
            }
        except (KeyError, TypeError, ValueError):
            return None
        if not values["normalized_text_sha256"] or values["normalized_len"] <= 0 or values["direct_len"] <= 0:
            return None
        return cls(
            **values,
    ...
    def matches(self, *, pdf_size: int, pdf_mtime_ns: int, normalized_text_sha256: str) -> bool:
        ...
        return (self.pdf_size, self.pdf_mtime_ns, self.normalized_text_sha256) == (
            int(pdf_size),
            int(pdf_mtime_ns),
            str(normalized_text_sha256 or "").strip(),
        )
...
from pipeline.core.stage_title_abstract import TitleAbstractStageMixin
from pipeline.core.stage_full_text import FullTextStageMixin
from pipeline.core.stage_data_extraction import DataExtractionStageMixin


class PaperScreeningPipeline(
    TitleAbstractStageMixin,
    FullTextStageMixin,
    DataExtractionStageMixin,
):
    def __init__(
        self,
        csv_dir: str,
        ...
        self._primary_scope_signal_pattern = topic_signal_config["primary_scope_pattern"]
        ...
        self._topic_primary_scope_terms = tuple(topic_signal_config.get("primary_scope_terms") or ())
        ...
        self._primary_action_pattern = self._primary_scope_signal_pattern
        self._monitoring_signal_terms: tuple[str, ...] = tuple()
        self._primary_action_terms: tuple[str, ...] = tuple()
        ...
        self._primary_action_pattern = (
            monitoring_signal_config.get("primary_action_pattern")
            or self._primary_scope_signal_pattern
        )
        self._monitoring_signal_terms = tuple(monitoring_signal_config.get("monitoring_terms") or ())
        self._primary_action_terms = tuple(
            monitoring_signal_config.get("primary_action_terms") or ()
        )
    ...
    def run(self) -> bool:
        ...
        stage_preparers = {
            "full_text": (self._materialize_paper_folders_full_text, self._full_text_pdf_dir),
            "data_extraction": (self._materialize_data_extraction_subset, self._data_extraction_pdf_dir),
        }
        if self.stage in stage_preparers:
            prepare_stage, stage_pdf_dir = stage_preparers[self.stage]
            prepare_stage()
            if self.split_only:
                pdf_dir = stage_pdf_dir()
                missing = self._find_missing_pdfs(pdf_dir)
                if missing and not self.quiet:
                    print(f"[prep] PDFs missing for {len(missing)} folder(s) in {pdf_dir.name}. Add one PDF per folder:")
                    for name in missing:
                        print(f"  - {name}")
        ...
        try:
            ...
            for idx, processed in enumerate(processed_stream, start=1):
                ...
                if self.stage in {"full_text", "data_extraction"} and not record.get("skip_selected_chunks_write"):
    ...
    def _use_async_stage_processing(self) -> bool:
        ...
        enabled = {
            "full_text": self._async_enable_full_text,
            "data_extraction": self._async_enable_data_extraction,
        }.get(self.stage, False)
        return bool(enabled and bool(use_api))
    ...
    async def _process_paper_async(self, paper: PaperRecord) -> tuple[dict, dict, dict | None]:
        ...
        if self.stage == "data_extraction":
            ...
            completed_payload = load_completed_output(
                output_dir.parent,
                output_dir.name,
                stage=self.stage,
            ...
            if completed_payload is not None:
                record = {
                    "paper_id": paper.paper_id,
                    "llm_decision": completed_payload.get("raw_output", ""),
                    "selected_chunks": [],
                    "diagnostics": {"completed_output_reused": True},
                    "metadata": paper.metadata,
                    "skip_selected_chunks_write": True,
                }
                token_stats = {
                    "prompt_tokens": 0,
                    "response_tokens": 0,
                    ...
                    "pdf_text_tokens": 0,
                    "pdf_visual_tokens": 0,
                    "prompt_tokens_source": "reused_completed_output",
                    "response_tokens_source": "reused_completed_output",
                    "embedding_tokens_source": "reused_completed_output",
                }
                return record, token_stats, completed_payload
        ...
        selection_trace: dict = {
            ...
            "topic_primary_scope_term_count": len(self._topic_primary_scope_terms),
            ...
            "primary_action_term_count": len(self._primary_action_terms),
    ...
    def _method_signal_flags(self, text: str) -> tuple[bool, bool, bool, bool]:
        """Extract prompt-driven primary/secondary cue flags used by full-text ranking."""

        value = (text or "").strip()
        has_primary_scope = bool(self._primary_scope_signal_pattern.search(value))
        ...
        has_all_topic_cues = has_primary_scope and has_primary_topic and has_secondary_topic
        return has_primary_scope, has_primary_topic, has_secondary_topic, has_all_topic_cues

    def _is_monitoring_only_text(self, text: str) -> bool:
        """Detect configured monitoring-like content that lacks explicit primary-scope mechanics."""
        ...
        has_primary_scope, has_primary_topic, has_secondary_topic, _ = self._method_signal_flags(value)
        ...
        has_primary_action = bool(
            self._primary_action_pattern.search(value)
            or has_primary_scope
            or self._primary_scope_signal_pattern.search(value)
        )
        return has_monitoring_cues and not has_primary_action
    ...
    def _hybrid_chunk_score(self, row: dict) -> float:
        ...
        has_primary_scope, has_primary_topic, has_secondary_topic, has_all_topic_cues = self._method_signal_flags(text)
        ...
        if has_all_topic_cues:
            hybrid_score += 0.08
        elif has_primary_scope and (has_primary_topic or has_secondary_topic):
            hybrid_score += 0.04
    ...
    def _method_priority_sort_key(self, row: dict) -> tuple[Any, ...]:
        ...
        has_primary_scope, has_primary_topic, has_secondary_topic, has_all_topic_cues = self._method_signal_flags(text)
        has_dual = has_primary_scope and (has_primary_topic or has_secondary_topic)
    ...
    def _infer_chunk_section_label(chunk: dict) -> str | None:
        ...
        heading, _ = ChunkBuilder._extract_section_heading(text_prefix)
        return heading
    ...
    def _call_llm(
        ...
        *,
        ...
        try:
            if use_api:
                response_format = self._response_format_for_llm(
                    extraction_schema=extraction_schema,
                    response_format_override=response_format_override,
                    use_extraction_response_format=use_extraction_response_format,
                )
    ...
    def _response_format_for_llm(
        self,
        *,
        extraction_schema: DynamicExtractionSchema | None,
        response_format_override: dict | None,
        use_extraction_response_format: bool,
    ) -> dict | None:
        """human readable hint: derive structured-output settings from the active schema once."""

        if response_format_override is not None or not use_extraction_response_format:
            return response_format_override
        ...
        if self.stage == "data_extraction" and active_schema is not None:
            return active_schema.openai_response_format()
        return None
    ...
    def _assess_borderline_decision(
        ...
        has_primary_scope_signal = bool(self._primary_scope_signal_pattern.search(signal_text))
        ...
        if (
            ...
            if has_primary_scope_signal and self._topic_primary_signal_pattern.search(signal_text):
                reasons.append("primary_topic_exclusion_with_scope_signal")
        ...
        if (
            ...
            if has_primary_scope_signal and self._topic_secondary_signal_pattern.search(signal_text):
                reasons.append("secondary_topic_exclusion_with_scope_signal")
    ...
    def _build_adjudication_context(
        ...
        if "primary_topic_exclusion_with_scope_signal" in reasons:
            reason_key = self._primary_topic_absence_reason_key or "primary-topic exclusion"
            guidance_lines.append(
                f"You previously excluded for {reason_key} despite primary-scope/topic cues; set this exclusion true only when methods clearly lack the required primary topic component."
            )

        if "secondary_topic_exclusion_with_scope_signal" in reasons:
            reason_key = self._secondary_topic_absence_reason_key or "secondary-topic exclusion"
            guidance_lines.append(
                f"You previously excluded for {reason_key} despite primary-scope/topic cues; set this exclusion true only when evidence clearly lacks the required secondary topic component."
            )
